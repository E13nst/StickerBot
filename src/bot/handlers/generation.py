        ...
        if wavespeed_client:
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to edit message with Document: {type(edit_error).__name__}")
                    
                    if query.inline_message_id:
                        # Для inline_message_id: НЕ fallback на photo, показываем текст
                        await context.bot.edit_message_text(
                            inline_message_id=query.inline_message_id,
                            text="✅ Generated (transparent file)",
                            reply_markup=keyboard,
                ...
                logger.warning(f"Error during document conversion: {type(conversion_error).__name__}")
                # Fallback на текст для inline, send_document для обычных сообщений
                if query.inline_message_id:
                    await context.bot.edit_message_text(
                        inline_message_id=query.inline_message_id,
